...
Click **Use this template** on the GitHub repo page. This creates a fresh repo under your account with no history from this project.

**Option B — Clone directly**

```bash
git clone https://github.com/jchaiet/quirkstart.git my-project
cd my-project
```
...
The setup script collects your credentials, writes both env files, generates a revalidation secret, and installs dependencies.
...
- Frontend and Studio URLs (defaults to `localhost`)
- SMTP credentials (optional — for form email delivery)

At the end it displays a generated `SANITY_REVALIDATE_SECRET` — copy this, you'll need it in step 5.

> **Manual setup:** If you prefer to configure env files yourself, copy `frontend/.env.example` to `frontend/.env.local` and `studio/.env.example` to `studio/.env`, then fill in the values. Run `pnpm install` afterwards.
...
2. Fill in:
   ...
   - **HTTP method**: POST
   - **Secret**: paste the `SANITY_REVALIDATE_SECRET` generated by the setup script
